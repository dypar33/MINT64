...
void jpeg_mcu_bitblt(int *src, int *dest, int width, int x0, int y0, int x1,
                     int y1)
{
    int w, h;
    int x, y, x2, y2;
    ...
    for (scan = 0; scan < jpeg->scan_count; scan++)
    {
        ...
        for (v = 0; v < vv; v++)
        {
            for (h = 0; h < hh; h++)
            {
                ...
                jpeg_mcu_bitblt(dest, p, jpeg->mcu_width, jpeg->mcu_width * h / hh,
