...
QWORD kExecuteProgram(const char *pcFileName, const char *pcArgumentString, BYTE bAffinity)
{
    ...
    QWORD qwEntryPointAddress, qwApplicationMemory, qwMemorySize;
...
static BOOL kLoadProgramAndRelocation(BYTE *pbFileBuffer, QWORD *pqwApplicationMemoryAddress, QWORD *pqwApplicationMemorySize, QWORD *pqwEntryPointAddress)
{
    ...
    int i;
    QWORD qwMemorySize, qwStackAddress;
    ...
    if ((pstELFHeader->e_ident[EI_MAG0] != ELFMAG0) || (pstELFHeader->e_ident[EI_MAG1] != ELFMAG1) || (pstELFHeader->e_ident[EI_MAG2] != ELFMAG2) || (pstELFHeader->e_ident[EI_MAG3] != ELFMAG3) || (pstELFHeader->e_ident[EI_CLASS] != ELFCLASS64) || (pstELFHeader->e_ident[EI_DATA] != ELFDATA2LSB) || (pstELFHeader->e_type != ET_REL))
    {
        return FALSE;
    }
...
static BOOL kRelocation(BYTE *pbFileBuffer)
{
    Elf64_Ehdr *pstELFHeader;
    Elf64_Shdr *pstSectionHeader;
    int i, j, iSymbolTableIndex, iSectionIndexInSymbol, iSectionIndexToRelocation, iNumberOfBytes;
    Elf64_Addr ulOffset;
    Elf64_Xword ulInfo;
