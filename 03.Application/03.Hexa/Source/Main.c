...
GAMEINFO g_stGameInfo = {
    0,
};

int Main(char *pcArgument)
{
    QWORD qwWindowID, qwLastTickCount;
    int iX, iY;
        ...
        if ((g_stGameInfo.bGameStart == TRUE) && ((GetTickCount() - qwLastTickCount) > (300 - (g_stGameInfo.qwLevel * 10))))
        {
            ...
            if (IsMovePossible(g_stGameInfo.iBlockX, g_stGameInfo.iBlockY + 1) == FALSE)
            {
                if (FreezeBlock(g_stGameInfo.iBlockX, g_stGameInfo.iBlockY) == FALSE)
                {
                    ...
                    DrawText(qwWindowID, 82, 230, RGB(255, 255, 255), RGB(0, 0, 0), "Game Over!", 10);
...
void Initialize(void)
{
    ...
    g_stGameInfo.iBlockX = -1;
    // g_stGameInfo.iBlockY = -1; bad code :(
}
...
void DrawInformation(QWORD qwWindowID)
{
    ...
    DrawRect(qwWindowID, 1, WINDOW_TITLEBAR_HEIGHT - 1, WINDOW_WIDTH - 2, WINDOW_TITLEBAR_HEIGHT + INFORMATION_HEIGHT, RGB(146, 184, 177), TRUE);
    ...
    if (g_stGameInfo.iBlockX != -1)
    {
        for (i = 0; i < BLOCKCOUNT; i++)
        {
            if (WINDOW_TITLEBAR_HEIGHT < (iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE)))
            {
                ...
                DrawRect(qwWindowID, g_stGameInfo.iBlockX * BLOCKSIZE, iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE), (g_stGameInfo.iBlockX + 1) * BLOCKSIZE, iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE), stColor, TRUE);

                stColor = g_stGameInfo.vstEdgeColor[g_stGameInfo.vbBlock[i]];
                DrawRect(qwWindowID, g_stGameInfo.iBlockX * BLOCKSIZE, iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE), (g_stGameInfo.iBlockX + 1) * BLOCKSIZE, iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE), stColor, FALSE);
                DrawRect(qwWindowID, g_stGameInfo.iBlockX * BLOCKSIZE + 1, iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE) + 1, (g_stGameInfo.iBlockX + 1) * BLOCKSIZE - 1, iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE) - 1, stColor, FALSE);
            }
        }
    }
